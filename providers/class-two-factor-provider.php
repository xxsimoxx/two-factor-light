...
abstract class Two_Factor_Provider {
	...
	public static function sanitize_code_from_request( $field, $length = 0 ) {
		if ( empty( $_REQUEST[ $field ] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended, handled by the core method already.
			return false;
		}

		$code = wp_unslash( $_REQUEST[ $field ] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, handled by the core method already.
